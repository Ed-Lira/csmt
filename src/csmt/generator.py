...
# Define a timing decorator for accurate GPU timing
class TimerContext:
...
def load_csm_1b(model_path="eustlb/csm-1b"):
    ...
    # Run warmup generation to compile CUDA graphs
    print("\nRunning warmup generation to compile CUDA graphs...")
    with TimerContext("Warmup generation"):
...
class Generator:
    def __init__(self, model, processor, device, debug=False):
        """
        ...
        debug : bool, optional
            Whether to print detailed debug information during generation, by default False
        """
        self.model = model
        ...
        self.gen_kwargs = DEFAULT_GEN_KWARGS.copy()
    ...
    def _generate_stream(
        ...
        """
        # Initialize RTF metrics tracking with CUDA events for accurate timing
            ...
            if token_count >= chunk_token_size or eos_generated:
                ...
                if max_len > 0:  # Only if we have tokens to process
                    ...
                    with torch.no_grad():
                        ...
                        avg_rtf = (
                                              chunk_end_time - start_time_total) / total_audio_duration if total_audio_duration > 0 else 0
        ...
        # Final summary stats - these are always printed as they're important for monitoring
        if chunk_count > 0:
        ...
        print(f"Total generation time: {elapsed_time:.4f} seconds")

    def generate_stream(self, inputs, chunk_token_size=1, **kwargs):
        """
